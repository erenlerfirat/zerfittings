...
export default function Dropdown({ children, trigger }) {
    ...
    return (
        <div className="w-fit relative bg-green-400 dark:text-white dark:border-0 rounded-3xl" ref={dropRef} onClick={() => setShow(curr => !curr)}>
            <div>{trigger}</div>
...
export function DropdownItem({ children, onClick }) {
    return (
        <li onClick={onClick} className="flex gap-3 items-center px-4 py-2 text-gray-800 hover:bg-gray-50 cursor-pointer">{children}</li>
    )
}